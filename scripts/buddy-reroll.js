...
//   --count <number>       Number of results to find (default: 5)
//   --check <uid>          Check what buddy a specific userID produces
//
// Examples:
//   node buddy-reroll.js --species duck --rarity legendary --shiny
...
function parseArgs() {
  ...
  const opts = { max: 50_000_000, count: 5 }
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--help': case '-h':
        console.log(`Usage: node buddy-reroll.js [options]
...
Options:
  ...
  --min-stats [value]    Require ALL stats >= value (default: 90)
  --max <number>         Max iterations (default: 50000000)
  --count <number>       Results to find (default: 5)
  --check <uid>          Check what buddy a specific userID produces`)
        process.exit(0)
    }
  }
...
for (let i = 0; i < opts.max; i++) {
  ...
  console.log(`#${found} [${r.rarity} ${RARITY_STARS[r.rarity]}] ${r.species} eye=${r.eye} hat=${r.hat} shiny=${r.shiny}`)
